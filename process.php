...
// Read env (Render supplies real env vars; .env optional locally)
...
if (!$SHEET_ID) { http_response_code(500); exit('Missing GOOGLE_SHEET_ID in env'); }
...
// Sanitize
$dateRaw  = clean((string)$_POST['selectedDate']);  // YYYY-MM-DD
...
// Dates
$tz = new DateTimeZone($_ENV['APP_TZ'] ?? 'Asia/Manila');
$dt = DateTime::createFromFormat('Y-m-d', $dateRaw, $tz);
if (!$dt) { http_response_code(400); exit('Invalid date format.'); }
$today = new DateTime('today', $tz);
if ((int)$dt->format('Ymd') < (int)$today->format('Ymd')) { http_response_code(400); exit('Selected date is in the past.'); }
$dayOfWeek = (int)$dt->format('w'); // 0=Sun, 6=Sat
if (!in_array($dayOfWeek, [0,6], true)) { http_response_code(400); exit('Reservations are available on weekends only.'); }

// Display strings
$prettyDate = $dt->format('l, F j, Y');
$timeObj    = DateTime::createFromFormat('H:i', $timeRaw, $tz);
...
if ($qrBinary === false) { http_response_code(500); exit('Failed to generate QR.'); }
...
<html lang="en"><head>
  ...
  <title>Reservation Confirmed • <?= $reservationNo; ?></title>
  ...
  <div class="w-full max-w-3xl">
    <div class="bg-white rounded-2xl shadow-2xl border border-orange-200 overflow-hidden">
      <div class="bg-orange-400 p-6 text-white flex items-center justify-between">
        ...
        <div class="text-sm">No: <span class="font-bold"><?= $reservationNo; ?></span></div>
      </div>

      <div class="p-6 grid md:grid-cols-2 gap-6">
        <div class="text-center">
          <img src="<?= $qrDataUri; ?>" alt="QR Code" class="mx-auto w-48 h-48 rounded-xl border border-gray-200 p-2 bg-white shadow" />
        ...
        <div>
          <div class="grid grid-cols-3 gap-x-4 gap-y-2 text-sm">
            ...
            <div class="text-gray-500">Guests</div><div class="col-span-2 font-medium"><?= $adults; ?> Adult<?= $adults>1?'s':''; ?><?= $kids>0?', '.$kids.' Child'.($kids>1?'ren':''):''; ?> (Total: <?= $totalGuests; ?>)</div>
            <?php if ($safeReq !== ''): ?>
              <div class="text-gray-500">Requests</div><div class="col-span-2 font-medium"><?= $safeReq; ?></div>
            <?php endif; ?>
          </div>
