...
<body class="bg-white min-h-screen">
  ...
  <div class="bg-white text-black py-16 px-4">
    <div class="max-w-4xl mx-auto text-center">
      ...
      <div class="mb-8 bg-white rounded-xl p-6 max-w-3xl mx-auto shadow-lg border border-gray-200">
        <h2 class="text-3xl font-bold text-center mb-6" style="color:#d0080e;">UNLI PALUTO PRICE</h2>
      ...
      <button onclick="openReservationModal()" class="bg-red-600 text-white px-8 py-4 rounded-full text-xl font-semibold hover:bg-red-700 transform hover:scale-105 transition-all duration-300 shadow-lg">
        Make a Reservation
      </button>
    </div>
  </div>
        ...
        <form id="reservationForm" action="process.php" method="post">
          ...
          <div class="mb-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <div class="flex items-center border border-gray-300 rounded-lg">
                  ...
                  <input type="number" id="adults" name="adults" value="10" min="1" max="20" readonly class="flex-1 text-center py-2 border-0 focus:ring-0">
            ...
            <div class="mt-2 text-sm text-gray-600">
              <span class="font-bold">Total guests:</span>
              <span id="totalGuests" class="font-medium">10</span>
              <span class="block text-xs text-gray-500">Minimum of 10 guests for online reservations. Children under 3? Their feast is free!</span>
            </div>

            <!-- Warning (shown when total < 10) -->
            <div id="minGuestsWarning" class="mt-2 text-sm text-red-600 hidden">
              Minimum of 10 guests required for online reservations.
            </div>
          </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-4 max-h-64 overflow-y-auto">
              <div class="space-y-4 text-sm text-gray-700">
                ...
                <ul class="ml-5 list-disc space-y-1 text-gray-700">
                    <li>₱599 UNLI-PALUTO is available every Saturday &amp; Sunday.</li>
                    <li>Each diner may upgrade their meal for an additional ₱100 to include either one (1) Shake or one (1) Dessert (not unlimited).</li>
                    <li>The ₱100 upgrade applies per diner and cannot be shared per table.</li>
                    <li>This offer is available for dine-in guests only.</li>
                    <li>Enjoy this promo every Saturday &amp; Sunday!</li>
                </ul>
                </div>
                <div>
                ...
                <ul class="ml-5 list-disc space-y-2 text-gray-700">
                    <li>
                    <span class="font-semibold text-black">On Your Birthday:</span>  
                    Enjoy FREE dining when accompanied by one (1) full-paying adult.
                    </li>
                    <li>
                    <span class="font-semibold text-black">Birthday Month Treat:</span>  
                    Dine for FREE anytime within your birthday month when accompanied by four (4) full-paying adults.
                    </li>
                </ul>
                </div>
                <div>
                ...
                <ul class="ml-5 list-disc space-y-2 text-gray-700">
                    <li>
                    <span class="font-semibold text-black">Dine-In Only:</span>  
                    Our unlimited offering is exclusively for dine-in. Sharing, take-outs, and bringing home leftovers are not allowed.
                    </li>
                    <li>
                    <span class="font-semibold text-black">No Leftovers:</span>  
                    To minimize food waste, excessive leftovers (200 grams or more per dish) will be charged ₱100 per 100 grams.
                    </li>
                    <li>
                    <span class="font-semibold text-black">No Outside Food & Drinks:</span>  
                    Guests are not permitted to bring outside food or beverages into the restaurant.
                    </li>
                </ul>
                </div>
                ...
                <ul class="ml-5 list-disc space-y-2 text-gray-700">
                    <li>
                    Guests must present a <span class="font-semibold text-black">valid government-issued or company ID</span> with photo and birthdate.
                    </li>
                    <li>
                    If no valid ID is available, a <span class="font-semibold text-black">birth certificate</span> together with a <span class="font-semibold text-black">photo ID</span> will be accepted.
                    </li>
                </ul>
                </div>
                ...
                <p class="text-gray-700">
                    The <span class="font-semibold text-black">Birthday Promo</span> may not be combined with any other discounts or promotions.
                </p>
                </div>
          ...
          <!-- Submit button (centered, with space from edges) -->
          <div class="mt-6 mb-8 flex justify-center">
            <button
              type="submit"
              class="inline-flex items-center justify-center w-full sm:w-auto md:w-64
                     bg-[#d0080e] hover:bg-[#b2070c] text-white py-3 px-8 rounded-lg font-semibold
  ...
  <!-- Walk-in suggestion modal -->
  <div id="walkinModal" class="fixed inset-0 bg-black/50 hidden z-[60] items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
      <h3 class="text-xl font-bold mb-2" style="color:#d0080e;">Walk-In Welcome</h3>
      <p class="text-gray-700 mb-4">
        Online reservations require a minimum of <strong>10 guests</strong>.
        For smaller groups, we’d love to serve you as a walk-in — plenty of tables are usually available at the venue.
      </p>
      <div class="flex justify-end gap-2">
        <button id="walkinClose" class="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200">Close</button>
        <button id="walkinOk" class="px-4 py-2 rounded-lg bg-[#d0080e] text-white hover:bg-red-700">Okay</button>
      </div>
    </div>
  </div>
  ...
  <script>
    (function(){
      ...
      let i=0, timer=null, isHover=false; const DURATION=5000;

      function setSlide(idx){
        i=(idx+images.length)%images.length;
        slide.style.transform='scale(1.03)';
        slide.src=images[i];
        requestAnimationFrame(()=>{setTimeout(()=>slide.style.transform='scale(1)',50)});
      ...
      function start(){ if(timer) clearInterval(timer); timer=setInterval(()=>{ if(!isHover) next() }, DURATION) }
      ...
      container.addEventListener('touchend', e=>{
        ...
        if(Math.abs(dx)>40) (dx>0?prev():next()); x0=null; start();
      },{passive:true});
      setSlide(0); start();
    })();
    ...
    function resetForm(){
      ...
      // defaults
      document.getElementById('adults').value='10';
      document.getElementById('kids').value='0';
      updateTotalGuests();
      document.querySelectorAll('.calendar-day').forEach(day=>{
        day.classList.remove('bg-orange-600','text-white');
        if(!day.disabled) day.classList.add('bg-gray-100','text-gray-800');
      });
      document.querySelectorAll('.time-slot').forEach(slot=>{
        slot.classList.remove('bg-orange-600','text-white');
    ...
    function changeCount(type, change){
      ...
      let n=v+change;
      if(type==='adults') n=Math.max(1,Math.min(20,n));
    ...
    function updateTotalGuests(){
      ...
      const kids=parseInt(document.getElementById('kids').value||'0',10);
      const total=adults+kids;
      document.getElementById('totalGuests').textContent=total;

      const warn=document.getElementById('minGuestsWarning');
      const belowMin = total < MIN_TOTAL;
      if(belowMin){
    ...
    function generateCalendar(){
      ...
      ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].forEach(d=>{
        ...
        h.className='text-center text-xs font-semibold text-gray-600 py-2';
        h.textContent=d; calendar.appendChild(h);
      });
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        el.type='button';
        el.className='calendar-day p-3 text-sm font-medium rounded-lg transition-all duration-200';
        el.textContent=day;
        const isWeekend=[0,6].includes(date.getDay());
        const isPast=date<today;
        if(isWeekend && !isPast){
          el.className+=' bg-gray-100 hover:bg-orange-100 border border-gray-300 text-gray-800 cursor-pointer hover:shadow-md';
          el.onclick=()=>selectDate(date,el);
        }else{
          el.className+=' bg-gray-50 text-gray-400 cursor-not-allowed';
          el.disabled=true;
        }
    ...
    function selectDate(date, element){
      document.querySelectorAll('.calendar-day').forEach(day=>{
        if(!day.disabled){
          day.classList.remove('bg-orange-600','text-white');
          day.classList.add('bg-gray-100','text-gray-800');
        }
      });
    ...
    document.getElementById('prevMonth').addEventListener('click',()=>{
      currentCalendarMonth--; if(currentCalendarMonth<0){currentCalendarMonth=11; currentCalendarYear--;} generateCalendar();
    });
    document.getElementById('nextMonth').addEventListener('click',()=>{
      currentCalendarMonth++; if(currentCalendarMonth>11){currentCalendarMonth=0; currentCalendarYear++;} generateCalendar();
    });

    document.querySelectorAll('.time-slot').forEach(slot=>{
      slot.addEventListener('click',function(){
        document.querySelectorAll('.time-slot').forEach(s=>{
          s.classList.remove('bg-orange-600','text-white'); s.classList.add('bg-gray-100');
        });
    ...
    document.addEventListener('DOMContentLoaded',()=>{ updateTotalGuests(); });

    // Walk-in modal handling + submit block if below minimum
    const walkinModal=document.getElementById('walkinModal');
    const walkinClose=document.getElementById('walkinClose');
    const walkinOk=document.getElementById('walkinOk');
    function openWalkinModal(){ walkinModal.classList.remove('hidden'); walkinModal.classList.add('flex'); }
    function closeWalkinModal(){ walkinModal.classList.add('hidden'); walkinModal.classList.remove('flex'); }
    walkinClose.addEventListener('click',closeWalkinModal);
    walkinOk.addEventListener('click',closeWalkinModal);
    walkinModal.addEventListener('click',e=>{ if(e.target===walkinModal) closeWalkinModal(); });
